...
def adjust_axes(arr1: np.ndarray, arr2: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if ndim2 > ndim1:
        raise ValueError("arr2 cannot have more dimensions than arr1 under this assumption")
...
## Misc
thr_window = lambda X, alpha=.4: X * (X > X.max()*alpha)
...
def split_job_from_arrays(len_arrays, n_split, stype="default"):
    ...
    if stype == "comb":
        mask = [list(np.where(i_split == mask % n_split)[0]) for i_split in range(n_split)]
    else:
...
def plot_array(ax:plt.Axes, x:np.ndarray, arr:np.ndarray, axis:int|None=None, **kwgs):
    if arr.ndim == 1:
        if len(x) != len(arr):
            raise ValueError(f"Dimension mismatch (x:{len(x)}, arr:{len(arr)}), array cannot be ploted")
            return None
        ax.plot(x, arr, **kwgs)
    else:
        if axis is not None:
            if len(x) != arr.shape[axis]:
                raise ValueError(f"Dimension mismatch (x:{len(x)}, arr:{arr.shape[axis]}), array cannot be ploted")
        else:
            for a in range(arr.ndim):
                if len(x) == arr.shape[a]:
                    axis = a
            if axis is None:
                raise ValueError(f"No matching dim, (x:{len(x)}, arr:{arr.shape}), array cannot be ploted")
...
def compute_sigma_2D(Y_m_t:np.ndarray, x_rec:np.ndarray, sig_in:float, sig_out:float, d_ax:np.ndarray, th_mem:float, l_elec:float, method=""):
    ...
    l_fem = x_rec[-1]
    e_mask = np.argwhere((x_rec > (l_fem-l_elec)/2) & (x_rec < (l_fem+l_elec)/2))
...
def get_samples_index(results:nerve_results, n_pts:int, alpha:float=0.001, t_iclamp:float=1, d_iclamp:float=0.2, n_pts_min=None)->np.ndarray:
    if "recorder" in results:
        ...
        i_t_samples = np.array([np.argmin(abs(Sdv_dt-(k*length_sample))) for k in range(n_pts)])
        ...
        if len(i_t_samples) < n_pts_min:
            i_t_samples = get_samples_index(results=results, n_pts=n_pts+1, alpha=alpha, t_iclamp=t_iclamp, d_iclamp=d_iclamp, n_pts_min=n_pts_min)
        return i_t_samples
...
def compute_v_rec_cap_idxs(Voltage, dt, t_stim=0, stim_duration=.2, tol=0.05, use_filter=True):
    ...
    if len(i_t_m)==0:
        ...
        i_t_min, i_t_max = np.argmin(Voltage[i_start_m:i_stop_m])+i_start_m, np.argmax(Voltage[i_start_m:i_stop_m])+i_start_m
    ...
    if use_filter:
        ...
        dv_rec_dt /= np.max(abs(dv_rec_dt)) 
    i_t_u = np.argwhere(dv_rec_dt>.1).squeeze()+i_start
    i_t_start, i_t_stop = i_t_u[0], i_t_u[-1]
    i_t_min, i_t_max = np.argmin(Voltage[i_t_start:i_t_stop])+i_t_start, np.argmax(Voltage[i_t_start:i_t_stop])+i_t_start
