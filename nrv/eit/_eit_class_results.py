...
class eit_class_results(dict):
    ...
    def load(self, data:str|dict):
        if isinstance(data, str):
            ...
            key_dic = data
        self.update(key_dic)
        np_keys = {"t_rec", "v_rec", "v_eit", "v_eit_phase", "failed_time_step", "t", "f", "p"}
        for key in np_keys:
            if key in self:
    ...
    def fail_results(self):
        ...
        if "failed_time_step" not in self:
            ...
            self["failed_time_step"] = np.squeeze(np.where((self.v_eit(i_e=0)==1e10) | (abs(self.dv_eit(i_e=0))>1e-3)))
    ...
    def v_eit_interp(self):
        if self._v_eit_interp is None:
            ...
            if not self.is_multi_freqs:
                self._v_eit_interp = nrv_interp(X_values=X_, Y_values=Y_, kind=self.interp_kind)

            else:
                Y_ = Y_.swapaxes(self.t_axis, self.f_axis)
                _interp = nrv_interp(X_values=X_, Y_values=Y_, kind=self.interp_kind)
                self._v_eit_interp = lambda X:_interp(X).swapaxes(self.t_axis, self.f_axis)
    ...
    def v_rec_interp(self):
        if self._v_rec_interp is None:
            X_ = self["t_rec"]
            Y_ = self["v_rec"]
            self._v_rec_interp = nrv_interp(X_values=X_, Y_values=Y_, kind=self.interp_kind)
        return self._v_rec_interp

    def incorporate_nerve_res(self, nerve_res:nerve_results|str|dict|dict|None):
        ...
        if isinstance(nerve_res, nerve_results):
            if "recorder" in nerve_res:
                ...
                for i in range(n_elec):
                    self["v_rec"][:, i] = np.array(nerve_res.recorder.recording_points[i].recording)
    ...
    def get_idxs(self, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None, n_t:int|None=None, include_freqs:bool=True, include_paterns:bool=True)->tuple[np.ndarray]:
        """
        ...
        """
        i_all = (
            set_idxs(i_t, n_t or self.n_t),
            set_idxs(i_e, self.n_e)
        )
    ...
    def ix_(self, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None, n_t:int|None=None,**kwgs)->tuple[np.ndarray]:
        i_ = self.get_idxs(i_t=i_t, i_e=i_e, i_f=i_f, i_p=i_p, n_t=n_t,**kwgs)
        return np.ix_(*i_)

    def v_eit_idx(self, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None, signed:bool=False, **kwgs):
        __filter_res = self.filter_res and i_t is None
        _v = self["v_eit"][self.ix_(i_t=i_t, i_e=i_e, i_f=i_f, i_p=i_p, **kwgs)].squeeze()
        if signed:
            sign = 2*(self["v_eit_phase"][self.ix_(i_t=i_t, i_e=i_e, i_f=i_f, i_p=i_p, **kwgs)].squeeze()<np.pi) - 1
            _v = _v*sign
    ...
    def v_eit(self, t=None, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None, **kwgs)->np.ndarray:
        if t is None:
    ...
    def v_0(self, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, **kwgs)->np.ndarray:
        kwgs.update({
            "i_t":0,
            "i_e":i_e,
            "i_f":i_f,
    ...
    def dv_eit(self, t:np.ndarray|float|None=None, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None, pc:bool=False,**kwgs)->np.ndarray:
    ...
    def dv_eit_pc(self, t:np.ndarray|float|None=None, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None,**kwgs)->np.ndarray:
        return self.dv_eit(t=t, i_t=i_t, i_e=i_e, i_f=i_f, i_p=i_p, pc=True,**kwgs)

    def dv_eit_normalized(self, t:np.ndarray|float|None=None, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None, pc:bool=False, axis:np.ndarray|int|None=None, **kwgs):
        _dv = np.abs(self.dv_eit(t=t, i_t=i_t, i_e=i_e, i_f=i_f, i_p=i_p, pc=pc,**kwgs))
        if axis is None:
            axis = self.t_axis
    ...
    def v_rec(self, t:np.ndarray|float|None=None, i_e:np.ndarray|int|None=None, **kwgs)->np.ndarray:
        if not self.has_nerve_res:
            print("No nerve results in object v_eit cannot be found")
            return None
        if t is None:
            _v = self["v_rec"]
            _v = _v[self.ix_(i_e=i_e, n_t=len(self["t_rec"]), include_freqs=False, include_paterns=False,**kwgs)]
            return _v
        else:
    ...
    def get_acap_mask(self, thr:float=0.05, t_new_cap:float|None=None,res_ax:Literal["default"]|None|tuple="default",**kwgs):
        # Computing normailzed dv
    ...
    def get_acap_t_ppt(self, thr:float=0.05, verbose:bool=False, store:Literal["default","overwrite","external"]="default", **kwgs)->pd.DataFrame:

        if store=="default" and self._cap_ppt is not None:
            return self._cap_ppt

        _cap_mask = np.swapaxes(self.get_acap_mask(thr=thr, **kwgs), self.e_axis, self.t_axis)
        
        # No CAP detected
        if not _cap_mask.any():
            print(f"No CAP detected. Check the results or increase the detection treshold (thr={thr})")
            return None
        ...
        i_cap_num = gen_idx_arange(np.where(np.diff(i_l,prepend=-1)>0)[0], i_l.shape[0])
    ...
    def get_acap_v_ppt(self, thr:float= 0.05, verbose:bool=False, store:Literal["default","overwrite","external"]="default",**kwgs)->pd.DataFrame:
        """
        ...
        dv_masked, _v_0 = self.get_dv_from_df(_cap_ppt, verbose=verbose, masked_time=True, with_v_0=True)
    ...
    def update_acap_inde_t_ppt(self, thr:float= 0.05, verbose:bool=False, store:Literal["default","overwrite","external"]="default",**kwgs)->pd.DataFrame:
        """
    ...
    def get_dv_from_df(self, data:pd.DataFrame, verbose:bool=False, masked_time:bool=True,with_v_0:bool=False)->np.ndarray|np.ma.MaskedArray:
        ...
        if masked_time:
            _i_t = np.multiply(np.ones(_v.shape, dtype=int), np.arange(self.n_t, dtype=int)[np.newaxis])
            ##! numpy masked array are inverted (i.e. only False element are considerated)
            _mask = ~((_i_t > data["i_t_min"].to_numpy()[:,np.newaxis]) & (_i_t < data["i_t_max"].to_numpy()[:,np.newaxis]))
    ...
    def get_reccap_ppt(self, thr:float=0.05, **kwgs):
        ...
        i_l = np.linspace(0,shape_2axes[0],2*shape_2axes[0],endpoint=False, dtype=int)

        i_reccap = np.zeros((2*shape_2axes[0],4), dtype=int) 
        for _i, _v in enumerate(_av_rec):
            i_reccap[2*_i,:], i_reccap[2*_i+1,:]= compute_v_rec_cap_idxs(_v, dt,)
        ...
        c_labs = ["i_l", *self._column_labels, "mye_cap", "i_start","i_min", "i_max", "i_stop"]
        c_labs.remove("i_f")
        self.rec_cap_ppt = pd.DataFrame(_da, columns=c_labs)

        self.rec_cap_ppt["durations"] = self["t_rec"][self.rec_cap_ppt["i_stop"]] - self["t_rec"][self.rec_cap_ppt["i_start"]]


        self.rec_cap_ppt["amplitudes"] = _av_rec[i_l,self.rec_cap_ppt["i_max"]] - _av_rec[i_l,self.rec_cap_ppt["i_min"]]
        return self.rec_cap_ppt

    # plot methods
    def plot(self, ax:plt.Axes, which:str="v_eit", t=None, i_t:np.ndarray|int|None=None, i_e:np.ndarray|int|None=None, i_f:np.ndarray|int|None=None, i_p:np.ndarray|int|None=None, pc=False, xtype="t", **kwgs):
        if t is None:
...
def load_res(res_dname:str, label:str)->eit_class_results:
    res = None
    if not os.path.isdir(res_dname):
        ...
        if os.path.isfile(fem_res_file):
            res = eit_class_results(data=fem_res_file)
        else:
            print(f"results file not found: {fem_res_file}")
    return res


def synchronize_times(res1:eit_class_results, res2:eit_class_results, dt_min=0.001)->np.ndarray:
    t1 = res1["t"]
    t2 = res2["t"]
