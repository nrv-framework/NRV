...
#define nrn_init _nrn_init__kds
#define _nrn_initial _nrn_initial__kds
#define nrn_cur _nrn_cur__kds
#define _nrn_current _nrn_current__kds
#define nrn_jacob _nrn_jacob__kds
#define nrn_state _nrn_state__kds
#define _net_receive _net_receive__kds 
#define states states__kds 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_kds", _hoc_setdata,
 "rates_kds", _hoc_rates,
 0, 0
};
#define rates rates_kds
 extern double rates( _threadargsprotocomma_ double );
 /* declare global and static user variables */
#define A_taux A_taux_kds
 double A_taux = 5;
#define B_taux B_taux_kds
 double B_taux = 0.022;
#define C_taux C_taux_kds
 double C_taux = 2.5;
#define Q10TempB Q10TempB_kds
 double Q10TempB = 10;
#define Q10TempA Q10TempA_kds
 double Q10TempA = 22.85;
#define Q10kds Q10kds_kds
 double Q10kds = 1.93;
#define S0p5y S0p5y_kds
 double S0p5y = -7;
#define S0p5x S0p5x_kds
 double S0p5x = 14.68;
#define V0p5y V0p5y_kds
 double V0p5y = -48;
#define Vpx Vpx_kds
 double Vpx = -65;
#define V0p5x V0p5x_kds
 double V0p5x = -39.59;
#define tau_y22 tau_y22_kds
 double tau_y22 = 7500;
 ...
 "Q10kds_kds", &Q10kds_kds,
 "Q10TempA_kds", &Q10TempA_kds,
 "Q10TempB_kds", &Q10TempB_kds,
 "V0p5x_kds", &V0p5x_kds,
 "S0p5x_kds", &S0p5x_kds,
 "A_taux_kds", &A_taux_kds,
 "B_taux_kds", &B_taux_kds,
 "C_taux_kds", &C_taux_kds,
 "Vpx_kds", &Vpx_kds,
 "V0p5y_kds", &V0p5y_kds,
 "S0p5y_kds", &S0p5y_kds,
 "tau_y22_kds", &tau_y22_kds,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kds /Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/kds.mod\n");
...
/*CVODE*/
 ...
 Dy1 = Dy1  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau_y )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   rates ( _threadargscomma_ v ) ;
    x = x + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_x)))*(- ( ( ( xinf ) ) / tau_x ) / ( ( ( ( - 1.0 ) ) ) / tau_x ) - x) ;
    y1 = y1 + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_y)))*(- ( ( ( yinf ) ) / tau_y ) / ( ( ( ( - 1.0 ) ) ) / tau_y ) - y1) ;
   }
  return 0;
}
...
double rates ( _threadargsprotocomma_ double _lVm ) {
   double _lrates;
 tau_x = A_taux * exp ( - pow( ( B_taux ) , 2.0 ) * pow( ( _lVm - Vpx ) , 2.0 ) ) + C_taux ;
   xinf = 1.0 / ( 1.0 + exp ( ( _lVm - V0p5x + shiftkds ) / ( - S0p5x ) ) ) ;
   tau_y = tau_y22 ;
   yinf = 1.0 / ( 1.0 + exp ( ( _lVm - V0p5y + shiftkds ) / ( - S0p5y ) ) ) ;
   tau_x = tau_x * pow( Q10kds , ( ( Q10TempA - celsius ) / Q10TempB ) ) ;
   tau_y = tau_y * pow( Q10kds , ( ( Q10TempA - celsius ) / Q10TempB ) ) ;
...
static void _initlists(){
 ...
 _slist1[1] = &(y1) - _p;  _dlist1[1] = &(Dy1) - _p;
_first = 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/kds.mod";
static const char* nmodl_file_text = 
  ...
  "			V0p5x=-39.59 (mV)		:As defined by Schild 1994, zinf=1.0/(1.0+exp((V-V0p5z)/(-S0p5z))\n"
  "			S0p5x=14.68(mV)\n"
  "		\n"
  "		: Tau Variables\n"
