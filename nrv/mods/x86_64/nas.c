...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "rates_nas", _hoc_rates,
 0, 0
};
#define rates rates_nas
 extern double rates( _threadargsprotocomma_ double );
 /* declare global and static user variables */
#define A_tauh A_tauh_nas
 double A_tauh = 4.95;
#define A_taum A_taum_nas
 double A_taum = 1.5;
#define B_tauh B_tauh_nas
 double B_tauh = 0.0335;
#define B_taum B_taum_nas
 double B_taum = 0.0595;
#define C_tauh C_tauh_nas
 double C_tauh = 0.75;
#define C_taum C_taum_nas
 double C_taum = 0.15;
#define Q10TempB Q10TempB_nas
 double Q10TempB = 10;
#define Q10TempA Q10TempA_nas
 double Q10TempA = 22.85;
#define Q10nash Q10nash_nas
 double Q10nash = 1.5;
#define Q10nasm Q10nasm_nas
 double Q10nasm = 2.3;
#define S0p5h S0p5h_nas
 double S0p5h = -4.5;
#define S0p5m S0p5m_nas
 double S0p5m = 4.45;
#define Vph Vph_nas
 double Vph = -20;
#define V0p5h V0p5h_nas
 double V0p5h = -18;
#define Vpm Vpm_nas
 double Vpm = -20.35;
#define V0p5m V0p5m_nas
 double V0p5m = -20.35;
 ...
 "Q10nasm_nas", &Q10nasm_nas,
 "Q10nash_nas", &Q10nash_nas,
 "Q10TempA_nas", &Q10TempA_nas,
 "Q10TempB_nas", &Q10TempB_nas,
 "V0p5m_nas", &V0p5m_nas,
 "S0p5m_nas", &S0p5m_nas,
 "A_taum_nas", &A_taum_nas,
 "B_taum_nas", &B_taum_nas,
 "C_taum_nas", &C_taum_nas,
 "Vpm_nas", &Vpm_nas,
 "V0p5h_nas", &V0p5h_nas,
 "S0p5h_nas", &S0p5h_nas,
 "A_tauh_nas", &A_tauh_nas,
 "B_tauh_nas", &B_tauh_nas,
 "C_tauh_nas", &C_tauh_nas,
 "Vph_nas", &Vph_nas,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nas /Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/nas.mod\n");
...
double rates ( _threadargsprotocomma_ double _lVm ) {
 ...
 tau_m = A_taum * exp ( - pow( ( B_taum ) , 2.0 ) * pow( ( _lVm - Vpm ) , 2.0 ) ) + C_taum ;
   minf = 1.0 / ( 1.0 + exp ( ( _lVm - V0p5m + shiftnas ) / ( - S0p5m ) ) ) ;
   tau_h = A_tauh * exp ( - pow( ( B_tauh ) , 2.0 ) * pow( ( _lVm - Vph ) , 2.0 ) ) + C_tauh ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lVm - V0p5h + shiftnas ) / ( - S0p5h ) ) ) ;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/nas.mod";
static const char* nmodl_file_text = 
