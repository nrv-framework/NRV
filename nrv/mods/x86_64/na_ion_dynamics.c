...
#define nrn_init _nrn_init__naoi
#define _nrn_initial _nrn_initial__naoi
#define nrn_cur _nrn_cur__naoi
#define _nrn_current _nrn_current__naoi
#define nrn_jacob _nrn_jacob__naoi
#define nrn_state _nrn_state__naoi
#define _net_receive _net_receive__naoi 
#define state state__naoi 
...
#define _style_na	*((int*)_ppvar[3]._pvoid)
...
static void nrn_alloc(Prop* _prop) {
 ...
 nrn_promote(prop_ion, 3, 0);
 	...
 	_ppvar[3]._pvoid = (void*)(&(prop_ion->dparam[0]._i)); /* iontype for na */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "diam");
 	...
 	ivoc_help("help ?1 naoi /Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/na_ion_dynamics.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
 static double FARADAY = 96485.3;
static int _reset;
static char *modelname = "na ion dynamics";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dnai = - ina * 4.0 / FARADAY / diam * ( 1e4 ) ;
   Dnao = ( ina / FARADAY - D * ( 0.1 ) * ( nao - naoinf ) ) / theta * ( 1e4 ) ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dnai = Dnai  / (1. - dt*( 0.0 )) ;
 Dnao = Dnao  / (1. - dt*( ( ( ( ( - ( D * ( 0.1 ) )*( ( 1.0 ) ) ) ) ) / theta )*( ( 1e4 ) ) )) ;
  return 0;
}
 /*END CVODE*/
 static int state (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    nai = nai - dt*(- ( ( ( ( ( - ina )*( 4.0 ) ) / FARADAY ) / diam )*( ( 1e4 ) ) ) ) ;
    nao = nao + (1. - exp(dt*(( ( ( ( - ( D * ( 0.1 ) )*( ( 1.0 ) ) ) ) ) / theta )*( ( 1e4 ) ))))*(- ( ( ( ( ( ina ) / FARADAY - ( ( D )*( ( 0.1 ) ) )*( ( ( - naoinf ) ) ) ) ) / theta )*( ( 1e4 ) ) ) / ( ( ( ( ( - ( ( D )*( ( 0.1 ) ) )*( ( 1.0 ) ) ) ) ) / theta )*( ( 1e4 ) ) ) - nao) ;
   }
  return 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/na_ion_dynamics.mod";
static const char* nmodl_file_text = 
