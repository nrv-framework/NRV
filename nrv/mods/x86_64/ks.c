...
#define rates rates_ks
 extern double rates( _threadargsprotocomma_ double );
 /* declare global and static user variables */
 static int _thread1data_inuse = 0;
static double _thread1data[3];
#define _gth 0
#define a_ks _thread1data[0]
#define a _thread[_gth]._pval[0]
#define b_ks _thread1data[1]
#define b _thread[_gth]._pval[1]
#define kvot_qt_ks _thread1data[2]
#define kvot_qt _thread[_gth]._pval[2]
#define lj lj_ks
 double lj = 0;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 ks /Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/ks.mod\n");
...
/*CVODE*/
 ...
 rates ( _threadargscomma_ v ) ;
 Dns = Dns  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau_ns )) ;
 Dnf = Dnf  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau_nf )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   rates ( _threadargscomma_ v ) ;
    ns = ns + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_ns)))*(- ( ( ( ninfs ) ) / tau_ns ) / ( ( ( ( - 1.0 ) ) ) / tau_ns ) - ns) ;
    nf = nf + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_nf)))*(- ( ( ( ninff ) ) / tau_nf ) / ( ( ( ( - 1.0 ) ) ) / tau_nf ) - nf) ;
   }
  return 0;
}
...
double rates ( _threadargsprotocomma_ double _lVm ) {
   double _lrates;
 ninfs = 1.0 / ( 1.0 + exp ( - ( _lVm + 30.0 ) / 6.0 ) ) ;
   ninff = 1.0 / ( 1.0 + exp ( - ( _lVm + 30.0 ) / 6.0 ) ) ;
   tau_ns = 1.0 * ( 13.0 * ( _lVm * 1.0 + lj ) + 1000.0 ) ;
   if ( _lVm < - 60.0 ) {
     tau_ns = 219.0 ;
     }
   a = 0.00395 * exp ( ( _lVm + 30.0 ) / 40.0 ) ;
   b = 0.00395 * exp ( - ( _lVm + 30.0 ) / 20.0 ) ;
   tau_nf = 1.0 / ( a + b ) ;
   kvot_qt = 1.0 / ( ( pow( 3.3 , ( ( celsius - 21.0 ) / 10.0 ) ) ) ) ;
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  ns = ns0;
 {
   ns = 1.0 / ( 1.0 + exp ( - ( v + 30.0 ) / 6.0 ) ) ;
   nf = 1.0 / ( 1.0 + exp ( - ( v + 30.0 ) / 6.0 ) ) ;
   }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/ks.mod";
static const char* nmodl_file_text = 
