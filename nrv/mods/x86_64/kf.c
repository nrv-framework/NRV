...
#define nrn_init _nrn_init__kf
#define _nrn_initial _nrn_initial__kf
#define nrn_cur _nrn_cur__kf
#define _nrn_current _nrn_current__kf
#define nrn_jacob _nrn_jacob__kf
#define nrn_state _nrn_state__kf
#define _net_receive _net_receive__kf 
#define states states__kf 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kf /Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/kf.mod\n");
...
double rates ( _threadargsprotocomma_ double _lVm ) {
   double _lrates;
 minf = pow( ( 1.0 / ( 1.0 + exp ( - ( 1.0 / km ) * ( v - vhm + lj + shift ) ) ) ) , 4.0 ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( 1.0 / kh ) * ( v - vhh + lj + shift ) ) ) ;
   tau_m = ( 0.25 + 10.04 * exp ( - ( pow( ( v + 24.67 ) , 2.0 ) ) / ( 2.0 * pow( 34.8 , 2.0 ) ) ) ) ;
   tau_h = ( 20.0 + 50.0 * exp ( - ( pow( ( v + 40.0 ) , 2.0 ) ) / ( 2.0 * pow( 40.0 , 2.0 ) ) ) ) ;
   if ( tau_h < 5.0 ) {
     tau_h = 5.0 ;
     }
   kvot_qt = 1.0 / ( ( pow( 3.3 , ( ( celsiusT - 23.0 ) / 10.0 ) ) ) ) ;
   tau_m = tau_m * kvot_qt ;
   tau_h = tau_h * kvot_qt ;
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  m = m0;
 {
   m = pow( ( 1.0 / ( 1.0 + exp ( - ( 1.0 / km ) * ( v - vhm + lj + shift ) ) ) ) , 4.0 ) ;
   h = 1.0 / ( 1.0 + exp ( ( 1.0 / kh ) * ( v - vhh + lj + shift ) ) ) ;
   }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/louisregnacq/Dropbox/Work/Model/NRV/NRV/nrv/mods/kf.mod";
static const char* nmodl_file_text = 
