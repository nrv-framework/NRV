...
if __name__ == "__main__":
    ...
    t_sim=3 # ms
    t_iclamp = 0 # ms
    dt_fem = t_sim/(3*n_proc_global[-1]-1) # ms


    parameters = {"x_rec":x_rec, "dt_fem":dt_fem, "l_elec":l_elec, "l_fem":l_fem, "i_drive":i_drive}

    fig, axs = plt.subplots(3, figsize=(8,6))

    for i in range(3):
        parameters["n_proc_global"] = n_proc_global[i]
        ...
        axs[0].plot(t, v_elecs, color="k", alpha=1-i*0.2)
        axs[1].plot(t, dv_elecs, color="k", alpha=1-i*0.2)
