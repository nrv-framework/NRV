...
param.add_domain(mesh_domain=1000,mat_pty="endoneurium_ranck")
...
res = sim1.solve_and_save_sim("./unitary_tests/results/outputs/127_results1")

param.add_domain(mesh_domain=1000,mat_pty=1)

sim2 = nrv.FEMSimulation(data=data, elem=('Lagrange', 1))
sim2.prepare_sim(jstim=jstim, _jstim=-jstim)

res -= sim2.solve_and_save_sim("./unitary_tests/results/outputs/127_results2")

res.save_sim_result("./unitary_tests/results/outputs/127_results", ftype="xdmf")
t2 = time.time()
