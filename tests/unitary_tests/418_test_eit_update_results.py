import eit
import matplotlib.pyplot as plt
import numpy as np

test_dir = "./unitary_tests/"
__fname__ = __file__[__file__.find(test_dir)+len(test_dir):]
test_id = __fname__[:__fname__.find("_")]

if __name__ == "__main__":

    res_fname  = f"./results/S0_3D_static_0_fem.json"
    fem_res = eit.eit_class_results(data=res_fname)

    print("m nerve sim time :", fem_res['computation_time'], "s")
    fem_res.filter_res = False
    print(fem_res.fail_results)

    i_e = np.arange(2)
    t =  fem_res.t()
    v_elecs = fem_res.v_eit(i_e=i_e)
    dv_elecs = fem_res.dv_eit(i_e=i_e)
    
    fig, axs = plt.subplots(3, 2, figsize=(12,6))
    axs[0,0].plot(t, v_elecs, ".-", color="r")
    axs[1,0].plot(t, dv_elecs, ".-", color="r")
    axs[2,0].plot(fem_res["t_rec"], fem_res["v_rec"][:,0], color="r")
    axs[0,0].set_ylabel("$V_{EIT}$ (V)")
    axs[1,0].set_ylabel("$dV_{EIT}$ (V)")
    axs[2,0].set_ylabel("$V_{REC}$ (mV)")
    axs[2,0].set_xlabel("time (ms)")
    fig.savefig(f"./unitary_tests/figures/{test_id}_m1_nerve.pdf")


    __v_eit = np.ones(fem_res.shape)
    __v_eit_phase = np.ones(fem_res.shape)
    fem_res.update_failed_results(_v_eit=__v_eit, _v_eit_phase=__v_eit_phase)


    t = fem_res.t(dt=0.05)
    v_elecs = fem_res.v_eit(t=t, i_e=i_e)
    dv_elecs = fem_res.dv_eit(t=t, i_e=i_e)


    axs[0,1].plot(t, v_elecs, ".-", color="r")
    axs[1,1].plot(t, dv_elecs, ".-", color="r")
    axs[2,1].plot(fem_res["t_rec"], fem_res["v_rec"][:,0], color="r")

    axs[0,1].set_ylabel("$V_{EIT}$ (V)")
    axs[1,1].set_ylabel("$dV_{EIT}$ (V)")
    axs[2,1].set_ylabel("$V_{REC}$ (mV)")
    axs[2,1].set_xlabel("time (ms)")
    fig.savefig(f"./unitary_tests/figures/{test_id}_m1_nerve.pdf")

    del fem_res
    plt.show()
