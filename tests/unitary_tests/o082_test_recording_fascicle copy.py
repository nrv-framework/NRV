...
if __name__ == '__main__':
    ...
    testrec2.set_recording_point(L/4, 0, 100)
    testrec2.set_recording_point(L/2, 0, 100)
    testrec2.set_recording_point(3*L/4, 0, 100)

    fascicle_1 = nrv.load_fascicle(source_file)
    fascicle_1.define_length(L)
    fascicle_1.set_axons_parameters(unmyelinated_nseg=4000)
    fascicle_1.set_ID(82)
    # intra cellular stimulation
    fascicle_1.insert_I_Clamp(position, t_start, duration, amplitude)
    ...
    for k in range(len(testrec2.recording_points)):
        # axs.append(plt.subplot(3,1,k+1))
        axs[k].plot(testrec2.t,testrec2.recording_points[k].recording)
        axs[k].set_xlabel('time (ms)')
        axs[k].set_ylabel('elec. '+str(k)+' potential (mV)')
        axs[k].set_ylim(-0.06,0.03)
        axs[k].set_xlim(0,15)
    ...
    # Fascicle declaration
    testrec2 = nrv.recorder('endoneurium_ranck')
    testrec2.set_recording_point(L/4, 0, 100)
    testrec2.set_recording_point(L/2, 0, 100)
    testrec2.set_recording_point(3*L/4, 0, 100)
    ...
    for k in range(len(testrec2.recording_points)):
        ...
        axs[k].grid()
    plt.tight_layout()
    plt.savefig('./unitary_tests/figures/082_C.png')