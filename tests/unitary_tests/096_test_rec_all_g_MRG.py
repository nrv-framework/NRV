...
if __name__ == "__main__":
    ...
    axon1 = nrv.myelinated(y,z,d,L,dt=0.005,rec='all', Nseg_per_sec=Nseg-1,model='MRG')
    ...
    plt.subplot(2,2,4)
    map = plt.pcolormesh(results['t'], results['x_rec'], results['g_i'] ,shading='auto')
    plt.xlabel('time (ms)')

    cbar = plt.colorbar(map)
    cbar.set_label('g_i (mV)')
    plt.savefig('./unitary_tests/figures/96_A.png')
    ...
    for i in range(8):
        ...
        plt.plot(results['t'],results['g_i'][x],label='g_i')
        ...
        plt.legend()
    plt.tight_layout()
    plt.savefig('./unitary_tests/figures/96_B.png')
    ...
    for i in range(8):
        ...
        plt.legend()
    plt.tight_layout()
    plt.savefig('./unitary_tests/figures/96_C.png')
